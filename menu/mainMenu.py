...
from game_core.story import Story
...
# initialize pictures
light = pygame.image.load('assets/images/spot.png')
light = pygame.transform.scale(light, (300, 300))
bg = pygame.image.load("assets/images/bg1.png")
...
def init_menu():
    ...
    footer_font = pygame.font.Font('assets/fonts/DeathFromAbove.ttf', 20)

    # initialize menu options
    first, space = 350, 60
    gameName="SPACE TANKS"#"GALACTIC TANK WAR"
    mainMenu.add(Option(lambda: get_option_text(gameName), 100, empty_func, title_font))
    mainMenu.add(Option(lambda: get_option_text("START"), first, start_game, menu_font))
    mainMenu.add(Option(lambda: get_option_text("OPTIONS"), (first + space), go_to_settings, menu_font))
    mainMenu.add(Option(lambda: get_option_text("EXIT"), (first + (space * 2)), sys.exit, menu_font))
    mainMenu.add(Option(lambda: get_option_text("Created    by  Sagar   Kumar"), 800, empty_func, footer_font))

    settingsMenu.add(Option(lambda: get_option_text(gameName), 100, empty_func, title_font))
